...
float camX = 300.0f, camY = 80.0f, camZ = 500.0f;
...
typedef struct _frame
{
	...
	float giY;		//Variable para GiroY
	float giZ;		//Variable para GiroZ
	float plane1 = -500.0f;
	float plane2 = 200.0f;
	float plane3 = 400.0f;
	float giroy = 0.0f;
...
void saveFrame(void)
{
	...
	std::cout << "X = " << plane1 << std::endl;
	std::cout << "Y = " << plane2 << std::endl;
	std::cout << "X = " << plane3 << std::endl;
	std::cout << "GiY = " << giroy << std::endl;
	std::cout << "GiZ = " << giroz << std::endl;


	KeyFrame[FrameIndex].plane1 = plane1;
	KeyFrame[FrameIndex].plane2 = plane2;
	KeyFrame[FrameIndex].plane3 = plane3;
...
{
	plane1 = KeyFrame[0].plane1;
	plane2 = KeyFrame[0].plane2;
	plane3 = KeyFrame[0].plane3;
...
{
	plane1inc = (KeyFrame[playIndex + 1].plane1 - KeyFrame[playIndex].plane1) / i_max_steps;
	plane2inc = (KeyFrame[playIndex + 1].plane2 - KeyFrame[playIndex].plane2) / i_max_steps;
	plane3inc = (KeyFrame[playIndex + 1].plane3 - KeyFrame[playIndex].plane3) / i_max_steps;
	giroyinc = (KeyFrame[playIndex + 1].giroy - KeyFrame[playIndex].giroy) / i_max_steps;
	girozinc = (KeyFrame[playIndex + 1].giroz - KeyFrame[playIndex].giroz) / i_max_steps;
...
void animate(void)
{
	...
	if (animacionPerro)
	{
		if (recorrido1)
		{
			if (movPatoY <= 30.0f) movPatoY += 1.0f;
			movAnimalx += 2.5f;
			orientaA = 90.0f;
		...
		if (recorrido3)
		{
			...
			if (movPatoY >= 1.0f) movPatoY -= 1.0f;
			movAnimalx -= 2.2f;
			orientaA = -90.0f;
			if (movAnimalx < -50.0f)
			{
...
int main()
{
	...
	Model casaFinal("resources/objects/casaFinal/casita.obj");
	Model heli("resources/objects/heli/AW101.obj");
	Model plane("resources/objects/plane/11805_airplane_v2_L2.obj");

	

	//Inicialización de KeyFrames
	KeyFrame[0].plane1 = -500.0f;
	KeyFrame[0].plane2 = 200.0f;
	KeyFrame[0].plane3 = 400.0f;
	KeyFrame[0].giroy = 0.0f;
	KeyFrame[0].giroz = 0.0f;

	KeyFrame[1].plane1 = -500.0f;
	KeyFrame[1].plane2 = 200.0f;
	KeyFrame[1].plane3 = 222.0f;
	KeyFrame[1].giroy = 7.0f;
	KeyFrame[1].giroz = -6.0f;

	KeyFrame[2].plane1 = -399.0f;
	KeyFrame[2].plane2 = 200.0f;
	KeyFrame[2].plane3 = 8.0f;
	KeyFrame[2].giroy = 14.0f;
	KeyFrame[2].giroz = -14.0f;

	KeyFrame[3].plane1 = -161.0f;
	KeyFrame[3].plane2 = 199.0f;
	KeyFrame[3].plane3 = -176.0f;
	KeyFrame[3].giroy = 14.0f;
	KeyFrame[3].giroz = -62.0f;

	KeyFrame[4].plane1 = 146.0f;
	KeyFrame[4].plane2 = 199.0f;
	KeyFrame[4].plane3 = -176.0f;
	KeyFrame[4].giroy = 14.0f;
	KeyFrame[4].giroz = -76.0f;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(glm::mat4(1.0f), glm::vec3(30.0f + movAnimalx, movPatoY, 0.0f + movAnimalz));
		...
		model = glm::translate(glm::mat4(1.0f), glm::vec3(-100.0f, 1.0f, -880.0f));
		...
		model = glm::translate(glm::mat4(1.0f), glm::vec3(plane1, plane2, plane3));
